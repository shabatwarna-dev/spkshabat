    ...
    <div class="card p-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="col-span-full">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-{{ min($teams->count(), 3) }} gap-2">
                    @foreach($teams as $team)
                    <label class="flex items-center gap-2.5 border-2 rounded-lg px-3 py-2.5 cursor-pointer transition-all
                                  {{ old('team_id') == $team->id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300' }}"
                           x-bind:class="selectedTeam == {{ $team->id }} ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'">
                        <input type="radio" name="team_id" value="{{ $team->id }}"
                               x-model="selectedTeam"
                               @change="updateNomorSPK({{ $team->id }})"
                               {{ old('team_id') == $team->id || ($loop->first && !old('team_id')) ? 'checked' : '' }}
            ...
            <template x-for="(proc, index) in processes" :key="proc.id">
                <div class="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div class="p-3 grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                        <div><label class="form-label">Est. Selesai</label><input type="date" :name="'processes['+index+'][estimasi_selesai]'" x-model="proc.estimasi_selesai" class="form-input text-xs"></div>
                        <div><label class="form-label">Jumlah Barang</label><input type="number" :name="'processes['+index+'][jumlah_barang]'" x-model="proc.jumlah_barang" class="form-input text-xs" placeholder="0" step="1" min="0"></div>
                        <div><label class="form-label">Satuan</label><input type="text" :name="'processes['+index+'][satuan]'" x-model="proc.satuan" class="form-input text-xs" placeholder="pcs / lembar"></div>
                        <div><label class="form-label">Montage</label><input type="text" :name="'processes['+index+'][montage]'" x-model="proc.montage" class="form-input text-xs" placeholder="1 Mata"></div>
                        <div><label class="form-label">Ukuran</label><input type="text" :name="'processes['+index+'][ukuran]'" x-model="proc.ukuran" class="form-input text-xs" placeholder="36x63 cm"></div>
                        <div><label class="form-label">Warna</label><input type="text" :name="'processes['+index+'][warna]'" x-model="proc.warna" class="form-input text-xs" placeholder="4 warna"></div>
                        <div><label class="form-label">Est. Hasil</label><input type="number" :name="'processes['+index+'][estimasi_hasil]'" x-model="proc.estimasi_hasil" class="form-input text-xs" placeholder="0" step="1" min="0"></div>
                        <div class="col-span-2"><label class="form-label">Catatan</label><input type="text" :name="'processes['+index+'][catatan_marketing]'" x-model="proc.catatan_marketing" class="form-input text-xs" placeholder="Catatan untuk tim produksi..."></div>
                    </div>
                </div>
            </template>
...
@push('scripts')
<script>
const teamNomors = @json($teams->mapWithKeys(fn($t) => [$t->id => \App\Models\ProductionOrder::generateNomorSPK($t)]));
...
function spkForm() {
    return {
        ...
        updateNomorSPK(teamId) {
            this.nomorSpk = teamNomors[teamId] || this.nomorSpk;
        }
    }
}
</script>
